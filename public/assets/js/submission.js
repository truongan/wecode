...
$(document).ready(function () {
	...
	$("td").on('click', '.btn', function () {
		...
		var view_code_request = $.ajax({
			...
			data: {
				...
				wcj_csrf_name: csrf_token
			},
			success: function (data) {
				if (type == 'code')
					 data.text = html_encode(data.text);
				...
				if (type == 'code'){
					// $('code.code-column').snippet(data.lang, {style: color_scheme});
		...
		if (!modal_open) {
			...
			$("#submission_modal").on("hidden.bs.modal", function(){
				$(".modal-body").html('<div style="text-align: center;">Loading<br><img src="'+base_url+'assets/images/loading.gif"/></div>');
				modal_open = false;
			});
	...
	$(".shj_rejudge").click(function () {
		...
		$.ajax({
			type: 'POST',
			url: site_url + 'rejudge/rejudge_single',
			data: {
				username: row.data('u'),
				assignment: row.data('a'),
				problem: row.data('p'),
			...
			success: function (response) {
				if (response.done) {
					...
					loading_failed(response.message);
			}
		});
	});
	$(".set_final").click(
