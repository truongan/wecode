...
document.addEventListener("DOMContentLoaded", function () {
	...
	$("td").on("click", "div.btn", function () {
		...
		); //First, clean up the modal body probably left over from previous view
		var button = $(this);
...
function update_status() {
	...
	$.ajax({
		...
		success: function (response) {
			...
			switch (response.status.toLowerCase()) {
				...
				case "score":
					if (response.judgement.mems.length > 0 && response.pre_score == 10000)
						row.querySelector(".js-time").innerHTML = Math.max(...response.judgement.times);
					if (response.judgement.mems.length > 0 && response.pre_score == 10000)
						row.querySelector(".js-mem").innerHTML = Math.max(...response.judgement.mems);
	...
	if (document.querySelectorAll("div.pending")) {
		setTimeout(update_status, update_status_interval);
	}
}
