...
class submission_controller extends Controller
{
	...
	{
		return Queue_item::whereHas("submission", function ($q) use ($user_id, $assignment_id, $problem_id) {
			$q->where(["user_id" => $user_id, "assignment_id" => $assignment_id, "problem_id" => $problem_id]);
		})->count() > 0;
	}
	...
	public function rejudge_view(Assignment $assignment)
	{
		...
		if (!$assignment->is_participant(Auth::user())) {
			abort(403, "You are not a participant to that assignment");
		}
	...
	public function get_template(Request $request)
	{
		...
		if ($template === null) {
			$result = ["banned" => "", "before" => "", "after" => "", "full" => ""];
		} else {
			...
			$result = ["banned" => $banned, "before" => $before, "after" => $after, "full" => $template];
		}
	...
	public function select_final(Request $request)
	{
		...
		if (!$submission_curr) {
			abort(403, "Submission not found");
		}
