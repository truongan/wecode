<table class="wecode_table table table-striped table-bordered table-sm">
	<thead class="thead-old table-dark">
		<tr>
			...
			<th>Total<br />accepted</th>
			@foreach ($problems as $problem)
				<th>
					<a
						class="small"
						href="{{ route('assignments.show', ['assignment'=>$assignment_id, 'problem_id'=> $problem->id]) }}"
						>{{ $problem->pivot->problem_name }}</a
					>
					...
					<a
						class="text-light"
						href="{{ route('submissions.index', ['assignment_id' => $assignment_id, 'problem_id' => $problem->id, 'user_id' =>'all' , 'choose' => 'final']) }}"
						>{{ $problem->pivot->score }}</a
					>
				</th>
			@endforeach
		</tr>
	...
	@foreach ($scoreboard["username"] as $i => $sc_username)
		<tr>
			...
			<td>
				...
				<p class="excess">
					<span class="small" title="Total Time + Submit Penalty">{{
						$scoreboard["submit_penalty"][$loop->index]
							->cascade()
			...
			<td class="bg-success text-white">
				...
				<p class="excess">
					<span class="small" title="Solved : Attack ratio"
						>{{ $scoreboard["solved"][$loop->index] }}:{{ $scoreboard["tried_to_solve"][$loop->index] }}</span
					>
				</p>
			</td>
			@foreach ($problems as $problem)
					...
					<p class="excess">
						...
						@if ($scores[$sc_username][$problem->id]["late"] > 0)
							<span class="text-warning">{{
								\Carbon\CarbonInterval::seconds($scores[$sc_username][$problem->id]["late"])
									->cascade()
									->forHumans(["short" => true])
							}}</span>
						@else
							<span class="small text-info">{{
								\Carbon\CarbonInterval::seconds($scores[$sc_username][$problem->id]["time"])
									->cascade()
									->forHumans(["short" => true])
							}}</span>
						@endif
	...
	<tfoot class="table-dark text-light">
		...
		<tr>
			...
			@foreach ($problems as $p)
				<td>{{ $stat_print[$p->id]->solved_tries }}</td>
			@endforeach
		</tr>
		<tr>
			<td colspan="6">Solved users/tries users/Total users</td>
			@foreach ($problems as $p)
				<td>{{ $stat_print[$p->id]->solved_tries_users }}</td>
			@endforeach
		</tr>
		<tr>
			<td colspan="6">Average tries per users</td>
			@foreach ($problems as $p)
				<td>{{ $stat_print[$p->id]->average_tries }}</td>
			@endforeach
		</tr>
		<tr>
			<td colspan="6">Average tries to solve</td>
			@foreach ($problems as $p)
				<td>{{ $stat_print[$p->id]->average_tries_2_solve }}</td>
			@endforeach
		</tr>
	</tfoot>
</table>
