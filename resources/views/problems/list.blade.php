...
@section("title_menu")
	...
	<a href="{{ route('problems.create') }}"
		><span class="ms-4 fs-6 text-dark-subtle"><i class="bi bi-plus fs-5 color8"></i> Add</span></a
	>
	<a href="{{ route('submissions.index',[0, Auth::user()->id, 'all', 'all'] ) }}"
		><span class="ms-4 fs-6 text-dark-subtle"><i class="bi bi-list-ul fs-5 color8"></i>Review practice submissions</span></a
	>
@endsection
			...
			<div class="accordion-item">
				...
				<div id="collapseOne" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
					<div class="accordion-body">
						<div class="row mb-2">
							<div class="col-md">
								<a href="" download id="download_all_selected"
									><i class="bi bi-download fs-5 text-info"></i>Export selected problem (zip)</a
								>
							</div>
							<div class="col-md">
								<a href="" id="select_all_for_download"><i class="bi bi-check-square fs-5 text-info"></i>Select all problems</a>
							</div>
							<div class="col-md">
								<a href="" id="deselect_all_for_download"><i class="bi bi-square fs-5 text-info"></i>Deselect all problems</a>
							</div>
							<div class="col-md-6">
								<form
									enctype="multipart/form-data"
									action="{{ route('problems.import') }}"
									method="post"
					...
					<table class="table table-striped table-bordered">
						...
						@foreach ($problems as $item)
							<tr data-id="{{$item->id}}">
								<td>
									{{
										($item->user->username == Auth::user()->username ? 0 : 0) +
											intval($item->id)
									}}
								</td>
								{{-- NAME --}}
							...
							"
								...
								<td>
									...
									@if ($item->can_edit(Auth::user()))
										<form action="{{ route('problems.edit_tags', $item->id) }}" method="post" class="edit-tag-form d-none">
											...
											<button type="button" class="btn btn-small btn-danger tags-edit-cancel">
										...
										<span class="edit-tag-list-handle">
											<i title="Edit tag list" class="bi bi-pencil-square fs-5 text-warning"> </i>
										</span>
									@endif
								</td>
								{{-- LANG --}}
								<td>
									<a
										...
										data-bs-toggle="collapse"
										href="#language_list_{{$item->id}}"
										aria-expanded="false"
										aria-controls="language_list_{{$item->id}}"
									>
										{{ $item->languages->pluck("name")->join(", ") }}
									</a>
									<div class="collapse" id="language_list_{{$item->id}}">
										@foreach ($item->languages as $language_name)
											<span class="btn btn-sm btn-secondary mb-1"
												>{{ $language_name->name }}
												<span class="badge rounded-pill bg-info">{{ $language_name->pivot->time_limit / 1000 }}s</span
												><span class="badge rounded-pill bg-info">{{ $language_name->pivot->memory_limit / 1000 }}MB</span></span
											>
										@endforeach
									</div>
								</td>
								...
								<td class="text-break">
									<span class="text-info">
										{{
											$item->updated_at
												? $item->updated_at->setTimezone($settings["timezone"])->locale("en-GB")->isoFormat("YYYY/MM/DD HH:mm:ss")
												: "Unknown"
										}}
									</span>
									<span class="text-muted">
										{{
											$item->created_at
												? $item->created_at->setTimezone($settings["timezone"])->locale("en-GB")->isoFormat("YYYY/MM/DD HH:mm:ss")
												: "Unknown"
										}}
									</span>
								</td>
								...
								<td>
									<a
										class="btn btn-sm btn-primary"
										data-bs-toggle="collapse"
										href="#assignment_list_{{$item->id}}"
										aria-expanded="false"
										aria-controls="assignment_list_{{$item->id}}"
									>
									...
									<div class="collapse" id="assignment_list_{{$item->id}}">
										@foreach ($item->assignments as $assignment)
											<a
												href="{{ route('submissions.index', ['assignment_id' => $assignment->id, 'problem_id' => $item->id, 'user_id' => 'all' , 'choose' => 'all']) }}"
											>
												<span class="btn btn-secondary btn-sm my-1"
													>{{ $assignment->name }}
													<span class="badge bg-info">{{ $assignment->user->username ?? "no-owner" }}</span>
												</span></a
											>
										@endforeach
									</div>
						...
						"
									...
									@if ($item->editorial != "")
										<a href="{{ $item->editorial }}" data-bs-toggle="tooltip" title="This problem has some linked editorial"
									...
									@if ($item->can_edit(Auth::user()))
										<a href="{{ route('problems.edit', $item) }}">
											<i title="Edit" class="bi bi-pencil-square fs-5 color3"> </i>
										</a>
...
@section("body_end")
	...
	<script>
		...
		function populate_search() {
			...
			tagsearch.slim.setData(
				all_tags.map((i) => {
					return { value: i.id, text: i.text };
				}),
			);
		...
		document.addEventListener("DOMContentLoaded", function () {
			...
			document.querySelectorAll(".edit-tag-list-handle").forEach((i) =>
			...
			document.querySelectorAll(".tags-edit-cancel").forEach((i) =>
				i.addEventListener("click", () => {
					// console.log(event.currentTarget);
			...
			document.querySelectorAll(".edit-tag-form").forEach((i) =>
				i.addEventListener("submit", (event) => {
					...
					fetch(event.currentTarget.action, {
						...
						.then((data) => {
							...
							event.target.parentElement.querySelector(".holder-for-one-problem-tags").innerHTML = data.new_tags
								.map((i) => '<span class="badge rounded-pill bg-info" data-id="' + i.id + '">' + i.text + "</span>")
								.join("");
