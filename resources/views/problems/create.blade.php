...
@section("body_end")
	...
	<script type="text/javascript">
		document.querySelectorAll(".add_language").forEach((x) =>
		...
		document.querySelectorAll(".remove_language").forEach((x) =>
			x.addEventListener("click", function () {
				var lang = this.dataset["lang"];
				var row = document.querySelector(".lang_row_" + lang);
				row.classList.toggle("d-none"); //.show();
...
@section("content")
	<div class=" ">
		...
		@if ($edit)
			<p>
				<i class="bi bi-info-circle-fill fs-5 color8"></i> If you don't want to change tests or pdf file, just do not upload its
				file.
			</p>
		@endif
		...
		>
					...
					<div class="row gy-3">
						<div class="form-floating">
							<input
								...
								value="{{ old('name',  $edit ? $problem->name : '') }}"
							/>
							@error("name")
						...
						<div class="form-floating">
							<input
								...
								name="author"
								class="form-control col-xs-7"
								value="{{ old('author',  $edit ? $problem->author : '') }}"
							/>
							@error("author")
								<div class="alert alert-danger" role="alert">
									<strong>{{ $message }}</strong>
						...
						<div class="form-floating">
							<input
								...
								name="editorial"
								class="form-control col-xs-7"
								value="{{ old('editorial',  $edit ? $problem->editorial : '') }}"
							/>
							@error("editorial")
								<div class="alert alert-danger" role="alert">
									<strong>{{ $message }}</strong>
							...
							@enderror
							<label for="editorial">Link to editorial </label>
						...
						<div class="d-none">
							...
							<div class="col-sm-7">
								<input
									...
									name="diff_cmd"
									class="form-control col-xs-7"
									value="{{ old('diff_cmd', $edit ? $problem->diff_cmd : 'diff') }}"
								/>
							</div>
						</div>
						<div class="d-none">
							...
							<div class="col-sm-7">
								<input
									...
									name="diff_arg"
									class="form-control col-xs-7"
									value="{{ $edit ? $problem->diff_arg : old('diff_arg', '-bB') }}"
								/>
							</div>
						</div>
						<div class="just-for-gutter">
							<label for="form_tests_dir"><i class="bi fs-5 bi-folder2-open"> </i>Tests and Descriptions (folder) </label>
							<input id="form_tests_dir" type="file" webkitdirectory multiple name="tests_dir[]" class="form-control" />
							<small class="text-secondary"
								>You can upload an entire folder of <strong> {{ $max_file_uploads }} </strong> file(s). If your test folder have
								more files you will have to upload a zip file of that folder instead. Also, this features is not web standard, some
						...
						<div class="just-for-gutter">
							...
							<input
								...
								max="5"
								name="difficult"
								value="{{ old('difficult',  $edit ? $problem->difficult : '') }}"
							/>
						...
						<div class="just-for-gutter">
							<label>Tag(s)</label>
							<small class="text-secondary"
								>You can add new tag by precedding them with '#' character (this character will not be present in the tag's
								text)</small
							>
				...
				<div class="col-sm-6">
					<div class="form-floating mb-3">
						<textarea
							...
							style="height: 10em"
							>{{ $edit ? $problem->admin_note : old("admin_note", "", false) }}</textarea
						>
					...
					<div class="row g-3 mt-4">
						<div class="form-check-inline form-check form-switch col">
							<input
								...
								{{
									($edit
											? $problem->allow_practice
											: old("allow_practice", 0))
										? "checked"
										: ""
								}}
							/>
						...
						<div class="form-check-inline form-check form-switch col">
							<input
								...
								id="sharable_switch"
								name="sharable"
								{{ ($edit ? $problem->sharable : old("sharable", 0)) ? "checked" : "" }}
							/>
							<label class="form-check-label" for="sharable_switch">
								Sharable
								<small class="text-secondary"
									>Allow other head_instructor to view this problem and use it in theirs assignments</small
								>
							</label>
						</div>
					</div>
					<div class="row g-3 mt-2">
						<div class="form-check-inline form-check form-switch col">
							<input
								...
								type="checkbox"
								class="form-check-input"
								id="allow_input_download_switch"
								name="allow_input_download"
								{{
									($edit
											? $problem->allow_input_download
											: old("allow_input_download", 0))
										? "checked"
										: ""
								}}
							/>
							<label class="form-check-label" for="allow_input_download_switch">
								Alow input download
								<small class="text-secondary"
									>Allow anyone that can submit to the problems to be able to download the input from testcases as well</small
								>
							</label>
						</div>
						<div class="form-check-inline form-check form-switch col">
							<input
								...
								type="checkbox"
								class="form-check-input"
								id="allow_output_download_switch"
								name="allow_output_download"
								{{
									($edit
											? $problem->allow_output_download
											: old("allow_output_download", 0))
										? "checked"
										: ""
								}}
							/>
							<label class="form-check-label" for="allow_output_download_switch">
								Allow output download
								<small class="text-secondary"
									>Allow anyone that can submit to the problems to be able to download the output from testcases as well</small
								>
							</label>
						</div>
					</div>
					<div class="row mt-4">
						<div class="form-check-inline form-check form-switch m-t-3">
							...
							<label class="form-check-label" for="customSwitch1"
								>Re-order files in <strong>in</strong> and <strong>out</strong> folder after upload<br /><small
									>This could useful when importing dataset from another format but should be use with care
								</small>
							</label>
						</div>
						<div class="my-4">
							...
							<small class="text-secondary"
								>Folder upload will always take precedent, if you want to upload zip file, leave upload folder field blank.
							</small>
						</div>
						<p>
							<a
			...
			@foreach ($all_languages as $lang)
				<a
					data-lang="{{ $lang->id }}"
					class="btn btn-success me-2 add_language add_language_{{ $lang->id }}
				{{ isset($languages[$lang->id]) ? "d-none" : "" }}"
					href="#!"
					role="button"
				>
					{{ $lang->name }}
							...
							@foreach ($all_languages as $lang)
								<tr class="lang_row_{{ $lang->id }} {{ isset($languages[$lang->id]) ? "" : "d-none" }}">
									<input
										...
										type="hidden"
										name="enable[]"
										value="{{ isset($languages[$lang->id]) ? "1" : "0" }}"
									/>
									...
									<td>
										<input
											...
											name="time_limit[]"
											class="form-control"
											value="{{ isset($languages[$lang->id]) ? $languages[$lang->id]->pivot->time_limit : $lang->default_time_limit}}"
										/>
									</td>
									<td>
										<input
											...
											name="memory_limit[]"
											class="form-control"
											value="{{ isset($languages[$lang->id]) ? $languages[$lang->id]->pivot->memory_limit :  $lang->default_memory_limit }}"
										/>
									</td>
									<td>
										<a
											data-lang="{{ $lang->id }}"
											class="btn btn-danger remove_language remove_language_{{ $lang->id }}"
											href="#"
											role="button"
