...
@section('title')
@if (isset($assignment)) 
	<h5 class="ml-2">Assignment: {{$assignment->name}}</h5>
@else
	<h5 class="ml-2">Problem: {problems|first.name}}</h5>
@endif
@endsection
@section('body_end')
...
<form action="{{ route('submit.huhuhuhuhuhuhuhu') }}" method="POST">
		@if (isset($assignment)) 
				...
				<select id="problems" name ="problem" class="form-control custom-select">
					@foreach ($problems as $problem)
                        <option value="{{ $problem->id }}">
                            #{{$problem->id}}--{{ $problem->problem_name }}
						</option>
					@endfor
				</select>
