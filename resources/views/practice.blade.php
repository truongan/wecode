			...
			<table class="table table-striped table-bordered">
				...
				@foreach ($problems as $problem)
					<tr data-id="{{$problem->id}}">
						...
						<td>
							<a
								href="{{ route('submissions.index', ['assignment_id' => 0, 'problem_id' => $problem->id, 'user_id' =>'all', 'choose' => 'all']) }}"
							>
								{{ $problem->accepted_submission }}/{{ $problem->total_submission }}</a
							>
						</td>
						<td>
							<a
								...
								href="#assignment_list_{{$problem->id}}"
								aria-expanded="false"
								aria-controls="assignment_list_{{$problem->id}}"
							>
							...
							<div class="collapse" id="assignment_list_{{$problem->id}}">
								@foreach ($problem->assignments as $assignment)
									<a
										href="{{ route('submissions.index', ['assignment_id' => $assignment->id, 'problem_id' => $problem->id, 'user_id' => 'all' , 'choose' => 'all']) }}"
									>
										<span class="btn btn-secondary btn-sm my-1"
											>{{ $assignment->name }} <span class="badge bg-info">{{ $assignment->user->username ?? "no-owner" }}</span>
										</span></a
									>
								@endforeach
							</div>
