...
@section("title_menu")
	...
	<ul class="ms-4 fs-6 nav nav-pills">
		<li class="nav-item">
			<a
				class="nav-link link-dark-subtle {{$place=="full" ? "active" :""}}"
				href="{{ route('scoreboards.index', $assignment->id) }}"
			>
				<i class="bi bi-star-fill color10"></i> Full information
			</a>
		</li>
		<li class="nav-item">
			<a
				class="nav-link link-dark-subtle {{$place=="simplify" ? "active" :""}}"
				href="{{ route('scoreboards.simplify', $assignment->id) }}"
			>
				<i class="bi bi-star-half color10"></i> Minimal information
			</a>
		</li>
		<li class="nav-item">
			<a
				class="nav-link link-dark-subtle {{$place=="plain" ? "active" :""}}"
				href="{{ route('scoreboards.plain', $assignment->id) }}"
			>
				<i class="bi bi-star-half color10"></i> Plain text Minimal
			</a>
		</li>
	</ul>
@endsection
