<nav id="mainNav" class="mainNav navbar navbar-expand-lg navbar-dark fixed-top color-{{ $selected }} bg-dark ">
	...
	<a class="navbar-brand" href="{{ route('home') }}" class="link-none"
		><img src="{{ asset('images/logo.png') }}" height="30px" />{{ $settings["site_name"] }}</a
	>
	<div class="navbar-collapse collapse" id="navbarCollapse">
		<ul
			class="navbar-nav navbar p-0 border-bottom-0 border-start-0 border-end-0 bg-dark navbar-dark navbar-sidenav color-{{ $selected }} align-items-start"
			id="exampleAccordion"
		>
			...
			<div class="sidenav-bottom nav-item mt-auto p-1">
		...
		<div class="navbar-nav ms-auto">
			@if (isset(Auth::user()->selected_assignment->name))
				<div class="d-flex flex-column justify-content-center bg-secondary bg-opacity-75 px-3">
		...
		<ul class="nav navbar-nav navbar-right">
			<li class="nav-item dropdown">
				...
				<button
					class="btn btn-link nav-link px-lg-2 dropdown-toggle d-flex align-items-center px-0 py-2"
					id="bd-theme"
					type="button"
					aria-expanded="false"
			...
			<li class="nav-item dropdown">
				<a class="dropdown-toggle nav-link" data-bs-toggle="dropdown" href="#" id="profile_link" rol="button" aria-expandd="false"
					><i class="bi bi-person-fill"></i>{{ Auth::user()->username }}</a
				>
						...
						<div class="">
							<div class="d-inline-flex">
								<form action="{{route('logout')}}" method="POST">
									@csrf
									<button type="submit" class="btn btn-danger me-2 text-nowrap">
										<i class="bi bi-box-arrow-right"></i>Sign out
									</button>
								</form>
								<a href="{{ route("users.show", Auth::user()->id) }}" class="btn btn-info text-nowrap"
									><i class="bi bi-wrench"></i>Profile</a
								>
							</div>
						</div>
					</div>
