	...
	<form action="{{ route('settings.update') }}" method="post">
		...
		<div class="form-old-row row">
			<fieldset class="col-md">
				...
				<input
					id="form_concurent_queue_process"
					type="number"
					name="concurent_queue_process"
					class="form-control"
					value="{{ $concurent_queue_process ?? 2 }}"
				/>
				<small class="form-text text-muted"
					>The number of queue process to run at the same time. This settings is sensitive to the server hardware capabilities. Set
					this to 0 will disable queue and stop judging all submissions</small
				>
			</fieldset>
			<fieldset class="col-md">
				<label for="form_default_language_number">Number of default languages</label>
				<input
					id="form_default_language_number"
					type="number"
					name="default_language_number"
					class="form-control"
					value="{{ $default_language_number ?? 2 }}"
				/>
			...
			<fieldset class="col-md">
				...
				<small class="form-text text-muted"
					>A list of space separated IP ranges that student (and instructor) are allowed to login from.
				</small>
			</fieldset>
		</div>

		<br />
		<h5>Judge settings</h5>
		<hr />
		<div class="g-4 row mb-3">
			<fieldset class="col-md">
				<label for="form_submit_penalty">Submit penalty</label>
				<input
					id="form_submit_penalty"
					type="number"
					name="submit_penalty"
					class="form-control medium"
					value="{{ $submit_penalty }}"
				/>
				<small class="form-text text-muted"
					>Penalty time (in seconds) for each Not accpeted submissions, in according to ICPC ruling</small
				>
			</fieldset>
			...
			<fieldset class="col-md">
				...
				<input
					id="form_results_per_page_all"
					type="number"
					name="results_per_page_all"
					class="form-control medium"
					value="{{ $results_per_page_all }}"
				/>
			...
			<fieldset class="col-md">
				...
				<input
					id="form_results_per_page_final"
					type="number"
					name="results_per_page_final"
					class="form-control medium"
					value="{{ $results_per_page_final }}"
				/>
		...
		<div class="g-3 row">
			...
			<fieldset class="col-md-3">
				...
				<div class="input-group">
					...
					<button
						...
						type="button"
						onClick="
							document.getElementById('form_reg_code').value =
				...
				<small class="form-text text-muted"
					>If you want to enable registration (above option), It is better to give a registration code to students in your class for
					validating registration. Enter 0 to disable.</small
				>
			</fieldset>

			<div class="col-md-3 form-floating">
				<input
					id="default_trial_time"
					type="number"
					name="default_trial_time"
					class="form-control medium"
					value="{{ $default_trial_time }}"
				/>
				<label for="default_trial_time">Default trial time</label>
				<small class="form-text text-muted"
					>The default trial time for newly added or newly registred students. Set to 0 to make all new user permanent
					student.</small
				>
			</div>
			...
			<div class="col-sm-4 form-floating">
				<textarea
					...
					rows="15"
					class="form-control add_text clear"
					style="height: 15em"
					>{{ $default_late_rule }}</textarea
				>
