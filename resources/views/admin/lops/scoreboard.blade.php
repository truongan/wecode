...
@section("title_menu")
	...
	@if (in_array(Auth::user()->role->name, ["admin", "head_instructor"]))
		<span
			class="ms-4 fs-6"
			onclick="
				document.querySelectorAll('a.btn-outline-success').forEach((i) => {
					console.log(i.classList);
					i.classList.remove('btn');
				})
			"
			><a href="#"><i class="bi bi-eraser text-success"></i>remove button (reload to restore)</a></span
		>
	@endif
@endsection
				...
				<thead class="thead-old table-dark">
					<tr>
						...
						@foreach ($lop->assignments as $ass)
							<th>
								<small>
									<a href="{{ route('assignments.show', ['assignment'=> $ass->id, 'problem_id' => '0'] ) }}"> {{ $ass->name }} </a>
								</small>
							</th>
						@endforeach
						<th><small>Sum</small></th>
						@foreach ($lop->assignments as $ass)
							<th><small> {{ $ass->name }} </small></th>
						@endforeach
					</tr>
				</thead>
							...
							<td class="">
								@if (isset($user_table[$user->id][$ass->id]["accept_score"]))
									<a
										...
										href="{{ route('submissions.index', ['assignment_id' => $ass->id, 'problem_id' => 'all', 'user_id' => $user->id, 'choose' => 'final']) }}"
									>
