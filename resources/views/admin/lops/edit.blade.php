...
@section("title_menu")
	<span class="ms-4 fs-6"
		><a href="{{ route('lops.index') }}"><i class="bi bi-list text-success"></i>Back to list of class</a></span
	>
	@if (in_array(Auth::user()->role->name, ["admin", "head_instructor"]))
		<span class="ms-4 fs-6"
			><a href="{{ route('lops.create') }}"><i class="bi bi-plus text-success"></i>Add class</a></span
		>
	@endif
@endsection
...
@section("content")
	@if ($ed == 0)
		@if ($lop->users()->find(Auth::user()->id))
			<form
				action="{{route('lops.enrol',['lop' => $lop->id, 'in' => 0])}}"
				method="POST"
				onsubmit="return confirm('You are going to LEAVE THE CLASS. Some classes may not allow you to re-enrol, are you certain?');"
			>
				@csrf
	...
	<form action="{{route('lops.update', $lop->id)}}" method="POST">
		...
		<fieldset>
			...
			<div class="custom-control custom-checkbox">
				...
				<small id="helpId" class="form-text text-muted" {{ $ed == 0 ? "hidden" : "" }}
					>User will be able to join any classes that are open for enrollment</small
				>
			</div>
			...
			<div class="table-responsive">
				...
				@if ($ed)
					<div class="form-group">
						...
						<textarea class="form-control" name="user_list" id="" aria-describedby="helpId" placeholder=""></textarea>
