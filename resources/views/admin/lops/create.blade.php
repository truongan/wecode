...
@section("content")
	<form action="{{route('lops.store')}}" method="POST" class="row g-3">
		...
		<div class="col-sm-6 form-floating">
			<input type="text" class="form-control" name="name" id="name" aria-describedby="_name_desc" placeholder="name" />
		...
		<div class="form-floating">
			<textarea style="height: 8em" class="form-control" name="user_list" id="" aria-describedby="helpId" placeholder=""></textarea>
			<label for="">List of users' names</label>
			<small id="helpId" class="form-text text-muted"
				>The list of usernames for every one enroll in the class, separated by comma, space, tab or</small
			>
		</div>
