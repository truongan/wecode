				...
				@foreach ($lops as $lop)
					<tr data-id="{{$lop->id}}">
							...
							{{
								$lop->users
									->filter(function ($item) {
										return in_array($item->role->name, ["admin", "head_instructor", "instructor"]);
									})
									->pluck("username")
									->join(", ")
							}}
						...
						<td>
							...
							@if (in_array(Auth::user()->role->name, ["admin", "head_instructor"]))
								<a title="Email all student" href={{ 'mailto:' . $lop->users->pluck('email')->join(',') }}>
								...
								<span title="Delete lop" class="delete-btn del_n delete_lop pointer" href="{{ route('lops.destroy', $lop->id) }}"
