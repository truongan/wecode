...
@section("title_menu")
	<a class="ms-4 fs-6 link-dark-subtle" href="{{ route('users.show', $user) }}">
		<i class="bi bi-person-fill-gear color2"></i>View user profile</a
	>
		...
		<div class="row g-3">
			<div class="card col-12">
				...
				<div class="card-body row">
					...
					<div class="col-sm-3">
						<p class=""><strong>Created: </strong>{{
							$user->created_at
								? $user->created_at->setTimezone($settings["timezone"])->locale("en-GB")->isoFormat("lll")
								: "N/A"
						}}</p>
						<p class=""><strong>Fisrt login: </strong>{{
							$user->first_login_time
								? $user->first_login_time->setTimezone($settings["timezone"])->locale("en-GB")->isoFormat("lll")
								: "N/A"
						}}</p>
					</div>
					<div class="col-sm-3">
						<p class=""><strong>last updated: </strong> {{
							$user->updated_at
								? $user->updated_at->setTimezone($settings["timezone"])->locale("en-GB")->isoFormat("lll")
								: "N/A"
						}}</p>
						<p class=""><strong>Last login: </strong>{{
							$user->last_login_time
								? $user->last_login_time->setTimezone($settings["timezone"])->locale("en-GB")->isoFormat("lll")
								: "N/A"
						}}</p>
					</div>
				</div>
			...
			<div class="col-sm-6 form-floating">
				...
				<small class="form-text text-small text-muted">
					This field would be checked only if you changed your own password. You have to supplied your old password in order to
					change it
				</small>
