...
@section("title_menu")
	...
	<a class="ms-4 fs-6 link-dark-subtle" href="{{ url('users/add_multiple') }}"
		><i class="bi bi-person-plus-fill text-success"></i> Add Users</a
	>
	<a class="ms-4 fs-6 link-dark-subtle" href="{{ 'mailto:' .  App\Models\User::pluck('email')->join(',') }}"
		><i class="bi bi-envelope-fill"></i> Email all users</a
	>
	<a class="ms-4 fs-6 link-dark-subtle" href="{{route('users.set_trial') }}"
		><i class="bi bi-highlighter"></i>Update multiple users trial time</a
	>
	{{-- <span class="ms-4 fs-6"><a href="{{ url('users/list_excel') }}"><i class="bi bi-download color9"></i> Excel</a></span> --}}
@endsection

@section("content")
	<a name="" id="copy_user_list" class="btn btn-primary my-2" href="#" role="button"
		><i class="bi bi-copy"></i> copy user name list</a
	>
	<div class="row">
	...
	<script>
		...
		function renderActions(row) {
			...
			return (
				...
				showUrl +
				'" class="bi bi-person-vcard fs-5 color0"></a> ' +
				'<a title="Edit" href="' +
				editUrl +
				'"><i class="bi bi-person-fill-gear fs-5 color9"></i></a> ' +
				'<a title="Submissions" href="' +
				subsUrl +
				'"><i class="bi bi-list fs-5 color12"></i></a> ' +
				...
				columns: [
					...
					{ data: "trial_end", orderable: false, searchable: false, render: $.fn.dataTable.render.text() },
					{
						...
						render: function (data) {
							return "<small>" + (data ? escapeHtml(data) : "Never") + "</small>";
						},
					},
					{
						data: "last_login",
			...
			$("table").on("click", ".delete-btn", function () {
				...
				$(".modal-body").html(
					"User ID: " +
						...
						"<br>Username: " +
						escapeHtml(username) +
						'<br><i class="splashy-warning_triangle"></i> All submissions of this user will be deleted.',
				);
						...
						success: function (response) {
							if (response.done) {
								if (!del_submssion) {
									...
									notify("All " + parseInt(response.count) + " submission(s) " + "of User " + username + " has been deleted.", {
