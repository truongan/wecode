...
@section("content")
	...
	<form method="post" name="main_form" class="row g-3">
		...
		<div class="col-sm-6 form-floating">
			<textarea
				...
				class="form-control"
				style="height: 15em"
				placeholder="Paste username list here"
				aria-describedby="help_names"
				value=""
				>{{ old("names") }}</textarea
			>
			<label for="names">Username lists</label>
			<small id="help_names" class="text-muted">A list of usernames, sperarated by white space to set their trial time to.</small>
		</div>
		<div class="col-sm-6">
			<div class="form-check">
				<input value="new_time" class="form-check-input" type="radio" name="set_choice" id="set_choice1" />
				<label class="form-check-label" for="set_choice1"> Set equal trial time for all users </label>
			</div>
			<div class="form-check">
				<input value="new_end" class="form-check-input" type="radio" name="set_choice" id="set_choice2" checked />
				<label class="form-check-label" for="set_choice2"> Set all users's trial time to end on the same date. </label>
			</div>
			<div class="form-floating my-2">
				<input type="number" class="form-control" name="new_trial_time" id="floatingInput" placeholder="name@example.com" />
	...
	@if (Session::has("success"))
		<div class="alert alert-success mt-3">Updated {{ session("success") }} user(s)</div>
	@endif
@endsection

@section("body_end")
	<script>
		document.main_form.set_choice.forEach((radio) =>
