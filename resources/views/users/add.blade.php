...
@section("title_menu")
	...
	<span class="ms-4 fs-6"
		><a href="{{ route('users.index') }}"><i class="bi bi-card-list color6"></i> Users list</a></span
	>
	<span class="ms-4 fs-6"
		><a href="https://github.com/truongan/wecode-judge/blob/docs/v1.4/users.md#add-users" target="_blank"
			><i class="bi bi-question-circle-fill color6"></i> Help</a
		></span
	>
@endsection
	...
	<script>
		document.querySelector("#add_users_button").addEventListener("click", function () {
			...
			fetch("{{ route("users.add") }}", {
				...
				if (response.status == 200) response.text().then((data) => (document.getElementById("main_content").innerHTML = data));
				else
...
@section("content")
	<div class="col-12">
		...
		<div class="form row">
			<label for="send_email" class="me-3"
				><input type="checkbox" name="send_mail" id="send_mail" />Send usernames and passwords by email</label
			>
			<label for="delay">
				(Waits <input type="number" size="5" name="delay" id="delay" class="form-control" style="width: 5rem" value="2" /> second(s)
				before sending each email, so please be patient).</label
			>
		</div>
	</div>
