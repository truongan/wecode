...
@section("title_menu")
	<a class="ms-4 fs-6 link-dark-subtle" href="{{ route('users.edit', $user) }}">
		<i class="bi bi-person-fill-gear color2"></i>Edit user profile</a
	>
...
@section("body_end")
	<script>
		...
		data2 = {
			...
			datasets: [{ values: {!! $hourly_data->pluck("count")->toJson() !!} }],
		};
		...
		new frappe.Chart("#pre_score", {
			data: {
				...
				datasets: [{ values: {!! $pre_score_data->pluck("count")->toJson() !!} }],
			},
			type: "bar",
...
@section("content")
		...
		<div class="col-4">
			<div class="input-group">
				...
				<input
					...
					name="display_name"
					class="form-control"
					value="{{ $user->trial_time ? ($user->created_at->addHours($user->trial_time)->diffForHumans()) : "Permanent user" }}"
					disabled
				/>
			</div>
		</div>
		<div class="col-4">
			<div class="input-group">
				...
				<input
					...
					name="display_name"
					class="form-control"
					value="{{ $user->first_login_time ? $user->first_login_time->setTimezone($settings['timezone'])->locale('en-GB')->isoFormat('llll (UZZ)') : 'Never'}}"
					disabled
				/>
			</div>
		</div>
	...
	<table class="table table-striped table-bordered table-sm">
		...
		<tr>
			<td>
				{{ @round( fdiv($stat['total_accept'] *100 , $stat['total_sub']),2) . "%" }}</td> <td>{{ @round(
				fdiv(count($stat['solved_problems'])*100 , count($stat['prob_wise'])) ,2 ) . "%"}}</td> <td>{{ @round(
				fdiv(array_sum($stat['solved_problems']) , count($stat['prob_wise']) ),2) }}</td> <td>{{ $user->elo ?? "" }}
			</td>
		</tr>
	</table>
			...
			<tr>
				...
				<td>
					@foreach ($as->ass->lops as $lop)
						<a href="{{ route('lops.show', $lop->id) }}">{{ $lop->name }}</a>
						</br>
					@endforeach
				</td>
				<td>
				...
				<td>
					<a
						...
						href="{{ route('submissions.index', ['assignment_id'=>$as->ass->id, 'problem_id'=>'all', 'user_id' => $user->id, 'choose'=>'final']) }}"
					>
				...
				<td>
					<a
						...
						href="{{ route('submissions.index', ['assignment_id'=>$as->ass->id, 'problem_id'=>'all', 'user_id' =>   $user->id, 'choose'=>'all']) }}"
					>
						{{ $as->score }}
					</a>
				</td>
