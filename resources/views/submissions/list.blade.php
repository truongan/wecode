			...
			<table class=" text-center table-responsive table table-bordered {{$choose == 'all' ? 'table-striped' : 'data-table'}}">
				<thead class="thead-dark">
					<tr>
						...
						@else
							...
							<th width="1%"><small> Code</small></th>
							<th width="1%"><small> Log</small></th>
				...
				<tr data-u="{{$submission->user->username}}" data-a="{{ $assignment->id }}" data-p="{{ $submission->problem_id }}" data-id="{{$submission->id}}">
					...
					<td>
						...
						<a href="{{route('submissions.index', [$assignment->id, $user_id, strval($submission->problem_id), 'all'])}}"><span class="btn btn-info btn-sm mx-1"><i class="fas fa-filter"></i></span></a>
					</td>
