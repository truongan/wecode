...
@section("title_menu")
	@if ($user_id != "all" and !in_array(Auth::user()->role->name, ["student", "guest"]))
		<a class="ms-4 fs-6 link-dark-subtle" href="{{route('submissions.index', [$assignment->id, 'all', $problem_id, 'all'])}}"
			>Remove filter user</a
		>
	@endif
	@if ($problem_id != "all")
		<a class="ms-4 fs-6 link-dark-subtle" href="{{route('submissions.index', [$assignment->id, $user_id, 'all', 'all'])}}"
			>Remove filter problem</a
		>
	@endif
@endsection
@section("body_end")
	...
	<script type="text/javascript">
		$("nav  > ul.pagination a").addClass("page-link");
	</script>
@endsection
@section("content")
	<div class="row row-cols-auto mb-2">
		<div class="col">
			@if ($choose == "all")
				<a
					href="{{route('submissions.index', [$assignment->id, $user_id, $problem_id, 'all'])}}"
				...
				<a
					style="opacity: 0.3"
					href="{{route('submissions.index', [$assignment->id, $user_id, $problem_id, 'final'])}}"
					class="btn btn-light active"
					role="button"
					>Final <i class="bi bi-chevron-right"></i
				></a>
			@else
				<a
					style="opacity: 0.3"
				...
				<a
					...
					class="btn btn-primary active"
					role="button"
					>Final <i class="bi bi-chevron-down"></i
				></a>
			@endif
			{{-- <hr> --}}
			@if ($choose == "all")
			...
			<table class=" text-center table-responsive table table-bordered {{$choose == 'all' ? 'table-striped' : 'data-table'}}">
				<thead class="thead-old table-dark">
					<tr>
						...
						<th width="1%">
							<small> Max <i class="bi bi-clock"></i>(s)</small>
						</th>
						<th width="1%">
							<small> Max <i class="bi bi-memory"></i>(kiB) </small>
						</th>
				...
				@foreach ($submissions as $submission)
					<tr
						data-u="{{$submission->user->username}}"
						data-a="{{ $assignment->id }}"
						data-p="{{ $submission->problem_id }}"
					...
					>
						...
						@if (!in_array(Auth::user()->role->name, ["student", "guest"]))
							<td>
								<a href="{{route('submissions.index', [$assignment->id, strval($submission->user_id), $problem_id, 'all'])}}">
									{{ $submission->user->username }} ({{ $submission->user->display_name }}) <br /><i class="bi bi-filter"></i> </a
								><br />
							</td>
							<td>
						...
						<td>
							@if ($assignment->id == 0)
								<a href="{{ route('practices.show', $submission->problem_id) }}"> {{ $submission->problem->name }}
							@else
							...
							></a>
							<a href="{{route('submissions.index', [$assignment->id, $user_id, strval($submission->problem_id), 'all'])}}"
								><span class="btn btn-info btn-sm m-1"><i class="bi bi-filter"></i></span
							></a>
						</td>
