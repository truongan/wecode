	...
	@else
		...
		<form action="{{ route('submissions.store') }}" method="POST" class="">
			...
			<div class="row row-cols-auto g-4 align-items-end">
				<div class="col">
					<label for="problems" class=""
						>Problem:<small class="form-text text-info"
							><a id="problem_link" href="#" target="_blank">View Problem statement</a></small
						></label
					>

					<select id="problems" name="problem" class="form-select">
						@if ($assignment->id == 0)
							<option value="{{ $problem->id }}" data-statement="{{ route('practices.show', $problem->id) }}">
						...
						@foreach ($assignment->problems as $prob)
							<option
								value="{{ $prob->id }}"
								data-statement="{{ route('assignments.show', [ 'assignment' =>$assignment->id , 'problem_id' => $prob->id])}}"
							>
								#{{ $prob->id }}--{{ $prob->pivot->problem_name }}
							</option>
						@endforeach
					</select>
			...
			<textarea style="display: none" rows="4" cols="80" name="code" class="sharif_input add_text"></textarea>
		</form>
	@endif
...
@section("body_end")
	...
	<script>
		...
		problem_languages[{{ $prob->id }}] = {!!
			$prob
				->languages()
				->whereIn("language_id", explode(", ", $assignment->language_ids))
				->get()
		!!};
