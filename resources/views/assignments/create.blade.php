...
@section("title_menu")
	<span class="ms-4 fs-6">
		<a href="https://github.com/truongan/wecode-judge/blob/docs/v1.4/add_assignment.md" target="_blank"
			><i class="bi bi-question-circle-fill text-danger"></i> Help</a
		>
		@if (Route::currentRouteName() == "assignments.edit")
			<a
				href="{{ route('submissions.index', ['assignment_id' => $assignment->id, 'problem_id' => 'all', 'user_id' =>'all', 'choose' => 'all']) }}"
			>
				<i class="bi bi-list color2"></i> Submissions list</a
			>
		@endif
	</span>
@endsection
...
@section("body_end")
	...
	<script
		integrity="sha384-uek4Os3EgXiGYKNUHPJzYoJkQhBOKlfIYq6OUOotrhUREP0mFW+62x2jbJXZPcv6"
		type="text/javascript"
		src="{{ asset('assets/js/add_assignments.js') }}"
	></script>
	<script>
		new SlimSelect({
			select: ".lop-select",
		...
		>
			...
			<div class="row">
					...
					<div class="form-old-row row gy-2">
						...
						<div class="col-sm-8">
							<input
								...
								value="{{ $edit ? $assignment->name : old('name') }}"
							/>
							@error("name")
							...
							<div class="form-old-row row">
								<div class="col-xl-7">
									<input
										...
										class="form-control"
										value="{{ $edit ? $assignment->start_time->setTimezone($settings['timezone'])->isoFormat('Y-MM-DD')  : old('start_time_date') }}"
									/>
								</div>
								<div class="col-xl-5">
									<input
										...
										class="form-control"
										value="{{ $edit ? $assignment->start_time->setTimezone($settings['timezone'])->isoFormat('HH:mm')  : old('start_time_time',0) }}"
									/>
								</div>
							</div>
						...
						<div class="col-sm-4">
							<label for="finish_time"
								...
								<small class="form-text text-muted"
									>Set finish time before start time will set the deadline to <strong>FOREVER</strong></small
								>
							</label>
						</div>
						<div class="col-sm-8">
							...
							<div class="form-old-row row">
								<div class="col-xl-7">
									<input
										...
										class="form-control"
										value="{{ $edit ? $assignment->finish_time->setTimezone($settings['timezone'])->isoFormat('Y-MM-DD') : old('finish_time_date') }}"
									/>
								</div>
								<div class="col-xl-5">
									<input
										...
										class="form-control"
										value="{{ $edit ? $assignment->finish_time->setTimezone($settings['timezone'])->isoFormat('HH:mm') : old('finish_time_time') }}"
									/>
								</div>
							</div>
						...
						<div class="col-sm-8">
							<input
								...
								id="extra_time"
								class="form-control"
								value="{{ $edit ? $assignment->extra_time : old('extra_time', '0*60*60') }}"
							/>
						...
						<div
							...
							title="If your classes have to take assignment with different start and/or finish time, please craete different assignments"
						>
							<label
								>Select class(es)<br />
						...
						<div class="col-sm-8">
							<select class="lop-select form-control" multiple="multiple" name="lop_id[]">
								@foreach ($all_lops as $p)
									<option value="{{ $p->id }}" data-name="{{$p->name}}" {{ isset($lops[$p->id]) ? 'selected="selected"' : "" }}>
										{{ $p->name }}
						...
						<div class="col-sm-4">
							<label for="form_pdf"
								>PDF File<br />
								<small class="form-text text-muted"
									>If the assignment has an additional PDF description file upload it here.
								</small>
							</label>
						</div>
						<div class="col-sm-8">
				...
				<div class="col-sm-6">
					...
					<textarea
						...
						name="description"
						rows="2"
						class="form-control add_text"
						>{{ $edit ? $assignment->description : old("description", "") }}</textarea
					>
					...
					<div class="form-check form-switch my-2">
						<input
							id="form_a_scoreboard"
							class="form-check-input"
							type="checkbox"
							name="scoreboard"
							value="1"
							{!! $edit ? ($assignment->score_board ? "checked" : "") : "" !!}
						/>
						<label for="form_a_scoreboard" class="form-check-label">Scoreboard</label>
						<small class="form-text text-muted mb-3"
							>Check this to publish scoreboard to student, lecturer can always see scoreboard</small
						>
					</div>
					{{-- {{ form_error('scoreboard', '<div class="alert alert-danger">', '</div>') }} --}}

					<label for="form_late_rule"
						>Coefficient rule (<a target="_blank" href="https://symfony.com/doc/current/reference/formats/expression_language.html"
							>Expression</a
						>)
						<small class="form-text text-muted">
							to calculate score coefficient (in percentage) based on <code>extra_time</code>, <code>delay</code> and
							<code>submit_time</code></small
						>
					</label>
					<input
						type="text"
						...
						rows="4"
						class="form-control add_text"
						value="{{ $edit ? $assignment->late_rule : old('late_rule', $settings['default_late_rule'], false) }}"
					/>
					...
					<div class="mt-2 row">
						<div class="col-sm-4">
							<label>
								Limit language
								<small class="form-text text-muted"
									>if your problems support many languages, you can limit the languages that can be used in this assigment
								</small>
							</label>
						</div>
						...
						<div class="col-sm-8">
							<select
								name="language_ids[]"
								class="languages-limit-select form-select"
								multiple
								aria-label="Further limit allow language for this assignments"
							>
			...
			<div class="row my-3">
				<div class="col-sm-8" id="choice_multi_assignment">
					<label>
						Select problem(s) for this assignment
						<small class="form-text text-muted">You can type in the box below to search for problems </small>
					</label>
					<select class="all_problems form-control" multiple="multiple">
						@foreach ($all_problems as $p)
							<option
								value="{{ $p->id }}"
								data-name="{{$p->name}}"
								data-sharable="{{$p->sharable}}"
								data-id="{{$p->id}}"
								data-tags="{{ $p->tags->implode('text', ', ')}}"
								data-note="{{ Str::limit($p->admin_note, 80) }}"
								data-no_of_assignment="{{ $p->assignments_count }}"
								data-owner="{{ $p->user->username ?? 'none'}}"
								{{ isset($problems[$p->id]) ? 'selected="selected"' : "" }}
							>
								{{ $p->id }} - {{ $p->name }} ({{ $p->user->username ?? "none" }} | {{ $p->tags->implode("text", ", ") }} | {{ $p->admin_note }})
							</option>
						@endforeach
					</select>
				</div>
				<div class="col-sm-4">
					...
					<small id="helpId" class="text-muted"
						>Use this to select every problems that have been used in no more than a specific number of assignments. You can still
						fine tuning the selection afterward
					</small>
				</div>
			</div>

			<label>
				Set alias, score and order for problems in this assignment
				<small class="form-text text-muted"
					>Problem's alias will be displayed when student view this assignment instead of the problem's original name<br />
					You can drag the handle to re-order the problems.<br />
					Remove one problem from assignment won't remove the submissions of that problem but will reset its alias and score to
					default if you re-add it later.
					...
					<br />Currently: <span class="badge bg-info count_problems">0</span> problems with a total score of
					<span class="badge bg-info sum_score">0</span>
				</small>
			</label>
			<ul id="problem_list" class="list-group">
				@php($i = 0)
				@foreach ($problems as $problem)
					<li class="list-group-item {{$problem->id == -1 ? 'd-none' : ''}} ">
						<div class="row align-items-center">
									...
									<div class="col">
										<div class="input-group input-group-sm me-2">
											...
											<input
												type="number"
												name="problem_score[]"
												class="form-control problem-score"
												value="{{ $problem->pivot->score }}"
											/>
										</div>
									</div>
							...
							<div class="col-auto">
								<button class="btn btn-danger list_remover">
									<span><i class="bi bi-x-circle-fill fs-5 pointer"></i></span>
								</button>
							</div>
							@php($i = $i + 1)
						</div>
