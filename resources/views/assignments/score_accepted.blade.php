...
@if (!in_array( Auth::user()->role->name, ['student', 'guest']))
    ...
    <small><nav class="nav nav-pills">
        ...
        <a class="nav-link active" href="{{ route('assignments.score_accepted') }}"><i class="bi bi-star-fill text-danger"></i>Assignments score accepted</a>
        <a class="nav-link" href="{{ route('assignments.score_sum') }}"><i class="bi bi-star-fill text-danger"></i>Assignments score olp</a>
    </nav></small>
    @endsection
@endif
@section('content')
